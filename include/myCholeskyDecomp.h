...
#ifndef MY_ROOT_Math_CholeskyDecomp
#define MY_ROOT_Math_CholeskyDecomp
...
public:
   ...
   CholeskyDecompGenDim(const CholeskyDecompGenDim& other)
           : fN(other.fN), fL(new F[(fN * (fN + 1)) / 2]), fOk(other.fOk)
   {
       std::copy(other.fL, other.fL + (fN * (fN + 1)) / 2, fL);
   }
   CholeskyDecompGenDim(CholeskyDecompGenDim&& other) :
       fN(other.fN), fL(other.fL), fOk(other.fOk)
   {
       other.fN = 0;
       other.fL = nullptr;
       other.fOk = false;
   ...
   CholeskyDecompGenDim& operator=(const CholeskyDecompGenDim& other)
   {
       if (this != &other) {
           if (fN < other.fN) {
               delete[] fL;
               fL = new F[(other.fN * (other.fN + 1)) / 2];
           }
           fN = other.fN;
           std::copy(other.fL, other.fL + (fN * (fN + 1)) / 2, fL);
           fOk = other.fOk;
   ...
   CholeskyDecompGenDim& operator=(CholeskyDecompGenDim&& other)
   {
       if (this != &other) {
           std::swap(fN, other.fN);
           std::swap(fL, other.fL);
           std::swap(fOk, other.fOk);
       }
       return *this;
   }
...
#endif // MY_ROOT_Math_CHOLESKYDECOMP
